...
public class ContentsCont {
  ...
  public ModelAndView create(int genreno, HttpSession session) {
  ...
  public ModelAndView list(@RequestParam(value = "word", defaultValue = "") String word,
    ...
    // 검색 목록
    List<ContentsVO> list = contentsProc.total_search(map);
    mav.addObject("list", list);

    // 검색된 레코드 갯수
    int search_count = contentsProc.total_search_count(map);
     ...
     * @return 페이징 생성 문자열
     */
    String paging = contentsProc.total_pagingBox("list_all.do", search_count, nowPage, word);
    mav.addObject("paging", paging);

    mav.addObject("nowPage", nowPage);

    mav.setViewName("/contents/list_all");
...
// http://localhost:9090/movie/contents/update.do
  ...
  public ModelAndView img_create(int contentsno, HttpSession session) {
    ModelAndView mav = new ModelAndView();
  ...
  public ModelAndView img_update(int contentsno, HttpSession session) {
    ModelAndView mav = new ModelAndView();
  ...
  public ModelAndView img_delete(HttpServletRequest request, int contentsno, int genreno, String passwd) {
    ModelAndView mav = new ModelAndView();
...
//http://localhost:9090/movie/contents/delete.do
  /**
   * 삭제 처리 + 파일 삭제
   * 
   * @param contentsVO
  ...
  public ModelAndView delete(HttpServletRequest request, int genreno, int contentsno, String passwd,
   ...
   */
  /**
   * 목록 + 검색 + 페이징 지원 http://localhost:9090/movie/contents/list.do
   * http://localhost:9090/movie/contents/list.do?genreno=1&word=스위스&nowPage=1
   * 
   * @param genreno
   * @param word
   * @param nowPage
   ...
   */
  @RequestMapping(value = "/contents/list.do", method = RequestMethod.GET)
  public ModelAndView list_by_genreno_grid1(@RequestParam(value = "genreno", defaultValue = "1") int genreno,
      @RequestParam(value = "word", defaultValue = "") String word,
  ...
  // http://localhost:9090/movie/contents/youtube_create.do?genreno=25&contentsno=28
  /**
   * Youtube 등록 폼
   * 
   * @return
  ...
  public ModelAndView youtube_create(int genreno, int contentsno, HttpSession session) {
    ModelAndView mav = new ModelAndView();
  ...
  // http://localhost:9090/movie/contents/youtube_create.do
  /**
   * Youtube 등록
   * 
  ...
  // http://localhost:9090/movie/contents/youtube_delete.do
  /**
   * Youtube 삭제 처리
   * 
   * @param genreno    카테고리 번호
  ...
  // http://localhost:9090/movie/contents/passwd.do?contentsno=28&passwd=123
  /**
   * 글의 패스워드 체크, JSON 출력
   * 
   * @return
  ...
  /**
   * 목록 + 검색 지원 http://localhost:9090/movie/contents/list.do
   * http://localhost:9090/movie/contents/list.do?genreno=1&word=스위스
   * 
   * @param genreno
   * @param word
   * @return
  ...
  /*    *//**
           ...
           *//*
              ...
              * contentsProc.list_by_genreno_search_paging(map); mav.addObject("list", list);
              * 
              * // 검색된 레코드 갯수 int search_count = contentsProc.search_count(map);
              * mav.addObject("search_count", search_count);
              * 
              ...
              * @return 페이징 생성 문자열
              * 
              * String paging = contentsProc.pagingBox("list.do", genreno, search_count,
  ...
  // http://localhost:9090/movie/contents/reply.do?genreno=1&contentsno=1
  /**
   * 답변 폼
   * 
   * @return
  ...
  // http://localhost:9090/movie/contents/reply.do
  /**
   * 답변 등록 처리
   * 
   * @return
