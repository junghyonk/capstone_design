...
COMMENT ON COLUMN contents.genreno is '카테고리 번호';
...
COMMENT ON COLUMN contents. is '출력 모드';
...
1) 글 등록(이미지 관련 컬러 추가)
-- : 1, genreno: 1이 사전에 등록이되어 있어야 INSERT 가능
...
VALUES(contents_seq.nextval, 1, 2, '스위스', '가을 여행', 'http://www.daum.net',
            '127.0.0.1', '123', '코로나', sysdate, 'le.jpg', 'le_t.jpg', 23657);
...
FROM contents
WHERE genreno=4 AND word LIKE '%스위스%'
ORDER BY contentsno DESC;

SELECT contentsno, , genreno, title, content, 
           recom, cnt, replycnt, rdate, word, file1, thumb1, size1
FROM contents
WHERE genreno=4 AND word LIKE '%스의스%'
ORDER BY contentsno DESC;

SELECT contentsno, , genreno, title, content, 
           recom, cnt, replycnt, rdate, word, file1, thumb1, size1
FROM contents
WHERE genreno=4 AND word LIKE '%수의스%'
ORDER BY contentsno DESC;
...
SELECT contentsno, , genreno, title, content, 
           recom, cnt, replycnt, rdate, word, file1, thumb1, size1
FROM contents
WHERE genreno=4 AND word LIKE '%유럽%'
ORDER BY contentsno DESC;
...
SELECT contentsno, , genreno, title, content, 
           recom, cnt, replycnt, rdate, word, file1, thumb1, size1
FROM contents
WHERE genreno=4 AND (title LIKE '%유럽%' OR content LIKE '%유럽%' OR word LIKE '%유럽%')
ORDER BY contentsno DESC;
...
SELECT contentsno, , genreno, title, content, recom, cnt, replycnt, rdate, word, ,
          file1, thumb1, size1
FROM contents
WHERE genreno=27 AND word LIKE '%단풍%'
ORDER BY contentsno DESC;
